...
params:
  ...
  count_data_file: "cohort1.rnaseq_raw_counts.rds" # file from GDCprepare
  # c_beta_file: "TARGET/NBL.gmm_phi.txt" # this is a 1 x n_tumor matrix with column headers being the tumor ids ### ALIGN THE TUMOR COLUMNS WITH c_beta FILE ###
  clinical_file: "clinical.processed.withAges.tsv"
  output_gct_file: "neuroblastoma_deseq_scale_factor"
  output_cls_file: "neuroblastoma_gmm_phi"
  phi_var: "phi"
---
...
out_dir <- file.path(proj_dir, 'GSEA')
...
```{r load_counts_data}
data <- readRDS(file.path(proj_dir, params$count_data_file))
is(data)
...
## Load the correlator data

We load the c-beta values here to ensure that the final normalized count matrix is in the same order as the phenotype.

```{r read_c_beta}
# c_beta_measured <- read.table(file.path(proj_dir, params$c_beta_file), sep = "\t", header = FALSE)
clinical <- read.table(file.path(proj_dir, params$clinical_file), sep='\t', header=T)
c_beta_measured <- clinical
# rownames(c_beta_measured) <- c_beta_measured$sampleID
c_beta_measured <- c_beta_measured[, c('sampleID', params$phi_var)]
...
# deseq_data <- rowsum(
#   as.matrix(data[keep, !(colnames(data) %in% c("gene_id", "gene_name"))]),
...
.createGCT(deseq_normal, pth = out_dir, nm = params$output_gct_file)
```

```{r save_cls_function}
.createCLS <- function(x, pth, nm) {
  filepath <- file.path(pth, paste0(nm, ".cls"))
  cat("#numeric\n", file = filepath, append = FALSE)
  cat(paste0("#", nm, "\n"), file = filepath, append = TRUE)
  cat(sprintf("Saving to path: %s\n", filepath))
...
.createCLS(c_beta_measured, pth = out_dir, nm = params$output_cls_file)
```