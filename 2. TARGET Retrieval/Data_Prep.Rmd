...
library(BiocFileCache) # Library import prevents odd errors with TCGAbiolinks
library(plyr)
library(jsonlite)
library(TCGAbiolinks)
library(sesameData) # Loading can prevent errors with TCGAbiolinks
...
if (as.character(packageVersion('TCGAbiolinks')) != TCGAbiolinks_version) {
  writeLines('Wrong version of TCGAbiolinks installed, need forked version...')
  writeLines('Please run "sh setup.sh" first!')
  knitr::knit_exit()
}
...
if (consts[["Windows"]]) {
  save_path <- file.path("P:", params$save_path)
} else {
...
```{r multiple_tumor_per_participant}
...
query_gene <- GDCquery(project = params$project, 
                       ...
                       sample.type = c("Primary Tumor"),
                       barcode = query_clinical$submitter_id[query_clinical$in_CpG_dataset])
...
GDCdownload(query_gene,
            directory = file.path(save_path, params$GDCdata_dir))
data.seq <- GDCprepare(query_gene,
                        directory = normalizePath(
                        file.path(save_path, params$GDCdata_dir),
                        winslash = "/",
                        mustWork = TRUE)
                       )
...
write.table(raw_counts, file = file.path(save_path, paste0(params$gene_filename, "_raw_counts.tsv")), 
            sep = "\t")
if (params$save_data_as_rds) saveRDS(raw_counts, 
                                     file = file.path(save_path, paste0(params$gene_filename, "_raw_counts.rds")))
dim(raw_counts)
...
GDCdownload(query_methyl, directory = file.path(save_path, params$GDCdata_dir))
...
write.table(methyl, 
            file = file.path(save_path, paste0(params$methyl_filename, ".tsv")), 
            sep = "\t")
if (params$save_data_as_rds) saveRDS(methyl, 
                                     file = file.path(save_path, paste0(params$methyl_filename, ".rds")))
...
write.table(query_clinical_flat, file = file.path(save_path, paste0(params$clinical_filename, ".tsv")), sep = "\t")
if (params$save_data_as_rds) saveRDS(query_clinical_flat, 
                                     file = file.path(save_path, paste0(params$clinical_filename, ".rds")))
```

```{r remove_gdc_data_direction, eval = params$save_data}
if (params$rm_GDCdata_dir) {
  unlink(file.path(save_path, params$GDCdata_dir), recursive = TRUE)
}
```
