...
getProcessedResponse <- function() {
  response <- readline('Permission to remove current installation of TCGAbiolinks (necessary for data retrieval) ([y]/n): ')
  response_processed <- stringr::str_to_lower(substr(response, 1, 1))
  response_processed
}
```
...
if (require('TCGAbiolinks', quietly=TRUE)) {
  if (as.character(packageVersion('TCGAbiolinks')) != TCGAbiolinks_version) {
    print('Wrong version of TCGAbiolinks installed, need forked version...')
    
        
    response_processed <- getProcessedResponse()
    if (!(response_processed == '' || response_processed == 'y')) {
      print('Permission not granted to remove current installation of TCGAbiolinks, quitting...')
      knitr::knit_exit()
    }
    

    print('Removing current installation of TCGAbiolinks...')
    remove.packages('TCGAbiolinks')
    install_forked = TRUE
  } else {
    print('HERE')
    print(packageVersion('TCGAbiolinks'))
    print(packageVersion('TCGAbiolinks') != TCGAbiolinks_version)
...
if (install_forked) {
    print('Installing forked version of TCGAbiolinks from danmonyak/TCGAbiolinks')
    print('Refrain from updating other packages (skip udates) unless necessary due to errors...')
    remotes::install_github("danmonyak/TCGAbiolinks")
    print('Installed!')
