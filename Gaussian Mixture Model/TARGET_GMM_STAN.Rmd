...
for (i in 1:nrow(tumor_params)) {
  ...
  psi_1_mean <- tumor_params$psi_1[i]
  psi_2_mean <- tumor_params$psi_2[i]
  psi_3_mean <- tumor_params$psi_3[i]
  X_mean     <- tumor_params$X[i]
  N_mean     <- tumor_params$N[i]
  ...
  print(fit, pars = c("psi[1]", "psi[2]", "psi[3]", "X", "N"))
  cat("\n\n")
