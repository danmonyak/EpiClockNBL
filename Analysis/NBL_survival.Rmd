...
km_plot <- autoplot(fit) +
  ...
  scale_color_discrete(labels = c(
    sprintf('Young (n = %d)', logrank$n[['dichom=Young']]),
    sprintf('Old (n = %d)', logrank$n[['dichom=Old']])
  )) +
...
ggsave(file.path('figures', "km_plot.png"), km_plot, width = sf * 6, height = sf * 5, dpi = 800)
...
ggsave(file.path('figures', "km_plot_4.png"), km_plot_4, width = sf * 6, height = sf * 5, dpi = 800)
...
coxResultsTable <- function(mod) {
  coef_table <- as.data.frame(summary(mod)$coefficients) # Get coefficients
  colnames(coef_table) <- c("Coef", "Exp(Coef)", "SE(Coef)", "z", "p") # Reformat
...
```{r check-confounding}
print(summary(coxph(Surv(time, event) ~ mitotic.age + cog_neuroblastoma_risk_group, data = survival_data)))
```
