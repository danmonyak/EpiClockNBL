...
dups <- duplicated(getParticipantIDs(common_tumors)) | 
...
#                          paste0(getParticipantIDs(common_tumors[dups]), "-01B"))

# Keep the first tumor (alphabetically)
common_tumors <- common_tumors[! duplicated(getParticipantIDs(common_tumors))]
```
There are `r length(common_tumors)` tumors with methylation data and CPE purity $\ge 0.6$; `r sum(duplicated(getParticipantIDs(common_tumors)))` of which are from the same participant.
...
cnv_participant_ids <- getParticipantIDs(cnv_cases)
cnv_tumor_ids <- getTumorIDs(cnv_cases)
...
print(colnames(methyl)[dups | duplicated(getTumorIDs(colnames(methyl)), fromLast = TRUE)])
...
keep_samples <- getTumorIDs(data.cnv$Sample) %in% common_tumors
